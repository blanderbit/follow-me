<template>
    <div id="news" >
        <div class="profil_my_data" v-for="item in news" style="margin-top: 20px;box-shadow: 0 3px 3px rgba(0,0,0,0.2);
-moz-box-shadow: 0 3px 3px rgba(0,0,0,0.2);
-webkit-box-shadow: 0 3px 3px rgba(0,0,0,0.2);">
          <!-- <div>{{ JSON.stringify(item) }}</div> -->
          <div class="show-news">{{item.date}} {{item.relation.firstname}} {{item.relation.lastname}}  </br>
           {{item.relation.photo}} </br>
            {{item.relation.quote}} </br>
            {{item.post}} </br>
            {{item.relation.lived}}
...
export default {
  data() {
    return {
      token: validToken(),
      animus:
        "http://img0.liveinternet.ru/images/attach/c/3/76/560/76560786_Clothing086_copy.png",
      news: [],
      comments: [],
      relation: null
    };
